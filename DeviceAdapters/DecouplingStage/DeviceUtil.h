...
public:
	...
	*/
	...
	std::string QueryParameter(std::string token);
	void SetParameter(std::string token, std::string value);
	...
	*/
	long MapProperty(PropertyAccessorWrapper propAcc, std::string description, bool readOnly = false, MM::PropertyType propType = MM::String, bool preInit = false);
...
template<typename T, typename U>
inline std::string DeviceUtil<T, U>::QueryParameter(std::string token)
{
	throw errorCode(DEVICE_UNSUPPORTED_COMMAND);
}

template<typename T, typename U>
inline void DeviceUtil<T, U>::SetParameter(std::string token, std::string value)
{
	throw errorCode(DEVICE_UNSUPPORTED_COMMAND);
}
...
template<typename T, typename U>
inline long DeviceUtil<T, U>::MapProperty(PropertyAccessorWrapper propAcc, std::string description, bool readOnly, MM::PropertyType propType, bool preInit)
{
	...
	CreateProperty(description.c_str(), val.c_str(), propType, readOnly, new CPropertyActionEx((U*) this, &DeviceUtil::OnProperty, id), preInit);
